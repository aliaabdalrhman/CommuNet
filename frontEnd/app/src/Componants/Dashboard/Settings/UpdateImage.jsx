...
export default function UpdateImage({ adminData }) {
    ...
    async function updateImage(values) {
        ...
        try {
            const { data } = await axios.post(`http://localhost:3700/userDo/uopdateimage//${localStorage.getItem('email')}`, formData, {
                headers: {
            ...
            toast.error(error.response.data.message || 'error in update your image');
        }
    }
    ...
    const deleteImage = (email) => {
        const confirmDelete = () => {
            if (window.confirm('are you sure to delete your image ?')) {
                const apiUrl = `http://localhost:3700/userDo/${email}/deleteImage`;
                ...
                <div {...getRootProps()}>
                    ...
                    {isDragActive ? (
                        ...
                        formik3.values.image ? (
                            <div>
                                <Avatar src={URL.createObjectURL(formik3.values.image)} alt="Uploaded" sx={{ width: 260, height: 260, mr: 4, borderRadius: 50 }} />
                                <p>Image uploaded</p>
                            </div>
                        ) : (
                            <> <div>
                                {adminData == null ? <Avatar sx={{ width: 260, height: 260, mr: 4 }} variant="circular">
                                </Avatar> : <Avatar src={adminData.secure_url} sx={{ width: 260, height: 260, mr: 4 }} variant="circular">
                                </Avatar>}
                            </div>
                                <Box sx={{ mt: 0, ml: 30 }}>
